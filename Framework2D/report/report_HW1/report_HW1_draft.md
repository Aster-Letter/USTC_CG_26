...
自由绘制模式下，`update()` 内部对路径点做最小距离过滤（阈值 2 像素），避免过密采样导致数据膨胀。
...
这样一次完整的拖拽或 slider 调整在 undo 历史中只占一个条目。
...
采用深拷贝而非浅拷贝的原因：编辑操作会原地修改图元对象的内部状态（坐标、样式等），如果 undo 历史与当前列表共享 `shared_ptr`，则历史快照也会被连带修改，导致丢失恢复点。深拷贝通过各图元的 `clone()` 实现，每个派生类返回自身的 `make_shared` 拷贝。
...
展示：Select 模式与绘制模式下属性面板的对比。

![图 7 上下文感知面板](images/07_context_panel.png)
